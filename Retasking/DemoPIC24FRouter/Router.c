...
void ProcessZigBeePrimitives(void)
{
    ...
    {
        case NLME_ROUTE_DISCOVERY_confirm:/*con tro khoi tao tuyen phat hien thiet bi khac*/
            #if defined(USE_DEBUG)
        ...
        case NLME_NETWORK_DISCOVERY_confirm:
            ...
            switch(params.NLME_NETWORK_DISCOVERY_confirm.Status)
            {
                case 0x00:
                        ...
                        printf("\r\n");
                    #endif
                    // Save the descriptor list pointer so we can destroy it later./*con tro ra nhap mang co san*/
...
SubmitJoinRequest:
                    /*con tro tham gia, tai tham gia mang dc chi dinh*/
                    params.NLME_JOIN_request.PANId = currentNetworkDescriptor->PanID;

                    #if defined(USE_DEBUG)
                        printf("Prepare join to network: 0x");/*thong bao cbi tham gia vao mang*/
